...
func (filter *TransactionFilter) filterByMessageOfMsgRun(messageValue MsgRun) bool {
	...
	if params.MRun.Package != nil {
		if params.MRun.Package.Name != nil && Deref(params.MRun.Package.Name) != messageValue.Package.Name {
			return false
		}
		if params.MRun.Package.Path != nil && Deref(params.MRun.Package.Path) != messageValue.Package.Path {
			return false
		}
	}
	return true

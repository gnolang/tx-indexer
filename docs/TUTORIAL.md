...
- Monitors `send` transactions in real-time
- Identifies the biggest GNOT transfers
- Serves transaction statistics via JSON API

## What You'll Learn

1. [Launch and configure the indexer](#step-1-getting-started---launch-your-own-indexer)
2. [Query blockchain data with GraphQL](#step-2-querying-for-send-transactions)
3. [Process and analyze transactions in Go](#step-3-processing-the-transaction-data-with-go)
4. [Stream real-time updates via WebSocket](#step-4-real-time-monitoring-with-websockets)
5. [Build a simple API](#step-5-api-implementation-example)

**Full code:** Available at [_assets/tx-indexer-example](_assets/tx-indexer-example)

**Try it first:** Use the hosted playground at [Test 10 GraphQL Playground](https://indexer.test10.testnets.gno.land/graphql) to experiment before setting up locally.
...
**Try it yourself!**
Copy that GraphQL query into the [playground](https://indexer.test10.testnets.gno.land/graphql) and execute it. You'll see real send transactions from the Gno network. 
...
To see a complete example implementation, visit [_assets/tx-indexer-example](_assets/tx-indexer-example).
